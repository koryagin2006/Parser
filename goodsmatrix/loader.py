...
def load_product(code):
    code = str(code)
    url = 'http://www.goodsmatrix.ru/goods/h/' + code + '.html'
    f = join(BASE, str(code) + '.html')
    if(isfile(f)):
        return
        
    info = get_html(url, {'User-Agent': choice(useragents)})
    if len(info) > 0 and info.find('<h1') != -1:
        with codecs.open('raw/products/' + code + '.html', 'w', encoding='utf-8') as f:
            f.write(info)
            f.close()
...
def main():
    ...
    reader = csv.reader(open('raw/code.csv', 'r'))
    for row in reader:
        if row[0] != 'ШТРИХКОД':
          load_product(row[0])
